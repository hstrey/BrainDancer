...
md"**Fit ellipse into averaged static volume**"
...
let img = staticimgs[:,:,sliceid]
	...
	# show image
	pseg = plot(Gray.(map(i->i-1, labels_map(seg)) |> genimg), aspect_ratio=1.0, axis=nothing, framestyle=:none, title="segments",)
...
Es = let n = sz[end],
	df = DataFrame(:x₀=>Float64[], :y₀=>Float64[],
				   :a=>Float64[], :b=>Float64[],
				   :θ=>Float64[], :A=>Float64[], :σ=>Float64[])
	img = similar(view(phantom_ts,:,:,:,1))
	for i in 201:n
		genunitimg!(img, view(phantom_ts,:,:,:,i))
		e = getellipse3d(img, verbose=false, secondfit=false)
		deleteat!(e, 6)
		push!(df, e)
	end
	df
...
defectidxs = findall(Es.a .< 0 .|| Es.b .< 0)
...
let p = @df dynamicE plot(:θ, labels="θ", legend=:topleft, title="Ellipse Rotation")
	plot!(p, fill(staticE[5],1:length(idxs)), color=:red, label="static μ")
end

# ╔═╡ e6deda84-ff89-4998-852d-cbf752d98936
md"**Perform image (ellipse-to-circle) transformation given the ellipse parameters from the static slices**"

# ╔═╡ 095d5158-c0f4-4019-9df4-99584127eb05
function ellipsetransform(E)
	rot = RotMatrix(E[5])
	scale = [1 0; 0 E[4]/E[3]]
	ori = E[1:2]
	(x) -> scale*rot*(x.-ori).+ori
end

# ╔═╡ fb4f9f88-ab07-4f76-b3b3-f77fd109796b
function transformimage(img; transfn=identity)	
	inter = ITI.Lanczos(3)
	# inter = ITI.BSpline(ITI.Constant())
	tmp = warp(img, transfn, axes(img), method=inter)
	mask = isnan.(tmp)
	mval = minimum(tmp[.!mask])
	tmp[mask] .= minimum(tmp[.!mask])
	tmp
end

# ╔═╡ 3ca69f60-09d4-45d1-aa6a-125d34808e62
let img = staticimgs[:,:,sliceid]
	imgw = transformimage(img; transfn=ellipsetransform([staticE[1:4]..., 0]))
	# show image
	pseg = plot(Gray.(imgw |> genimg), aspect_ratio=1.0, axis=nothing, framestyle=:none, title="EtoC transformed")
	pimg = plot(Gray.(genimg(img)), aspect_ratio=1.0, axis=nothing, framestyle=:none, title="img #$sliceid", size=(300,350))
	plot(pimg, pseg)
end

# ╔═╡ f5cedf8d-5b3b-499c-bea3-0eac7a37ab88
md"**Perform transormations on dynamic slices**"

# ╔═╡ 1129c80b-e198-4fa6-a158-a31fb9c193d0
imgds = let sz = size(idxs)
	res = zeros(size(phantom_ts)[1:2]...,sz...)
	for (i,j) in enumerate(idxs)
		E = [staticE[1:4]..., staticE[5]-dynamicE.θ[i]] # rotate to θₛ-θᵣ angle
		res[:,:,i] .= transformimage(view(phantom_ts,:,:,sliceid,j); transfn=ellipsetransform(E))		
	end
	res
end;

# ╔═╡ 77e717c5-80dc-458f-9179-1059dfdb965c
let images = []
	for i in 1:4
		pp = plot(Gray.(genimg(imgds[:,:,i])),aspect_ratio=1.0,axis = nothing,framestyle=:none,title="$i")	
		push!(images,pp)
	end
	plot((images...),layout=length(images))
end
...
md"**Generate images from the static avarage by rotating to an angle of a rotation of a dynamic slice**"

# ╔═╡ 360de0f7-aa53-4eda-902b-f0b8d3fd1f1e
md"""
Coordinates:
- X: $(@bind x html"<input type=number min=1 max=83 value=1></input>")
- Y: $(@bind y html"<input type=number min=1 max=84 value=1></input>")
- slice #: $(@bind sid html"<input type=number min=1 max=13 value=1></input>")
"""

# ╔═╡ 39ddb2af-3643-45ff-b8f0-f8c8e09bcbc7
imgss = let θs = dynamicE.θ, # rotation angles of dynamic volumes
			res = zeros(size(staticimgs)[1:2]...,length(θs))
	for (i,θ) in enumerate(θs)
		E = [staticE[1:4]..., staticE[5] - θ] # rotate to θₛ-θᵣ angle
		res[:,:,i] .= transformimage(view(staticimgs,:,:,sid); transfn=ellipsetransform(E))		
	end
	res
end;

# ╔═╡ f6b8e7cf-aecf-414c-a448-5e1ca39a00d4
let imgd = view(phantom_ts,:,:,sid,idxs)
	z1 = imgss[x,y,:]	
	p = plot(z1, label="simulated", legend=:right, right_margin=1.2cm)
	z2 = imgd[x,y,:]
	q = p #twinx(p)
	plot!(q, z2, label="dynamic (slice #$sid)", color=:red)
end
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "9892e94c1127d04ee8fd48105c460deae7e05097"
